...
let check_description table_name descr pgsql_descr =
  let check (correct, descr) field =
    let field_name = Sql.Value.get field#name in
    ...
    match field_type with
      ...
      | Some field_type ->
          ...
          let sql_type = Sql.sql_type_of_string (Sql.Value.get field#data_type) in
          let nullable =
            match Sql.Value.get field#nullable with
              | "YES" -> true
              | "NO" -> false
              | other ->
                  correct := false;
...
let check comp =
  match comp.Sql.concrete with
    | Sql.Selection q -> failwith "unsupported query check"
    | Sql.Table (schema, table) ->
        ...
        let check_result =
          try `Result (check_description table_name comp.Sql.descr
