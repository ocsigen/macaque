...
(** It is rather awkward to protect SQL identifiers here, at the
    ...
    convenient. *)
let keyword_safe warn identifier =
  ...
  if not (List.mem_assoc kw reserved_keywords) then identifier
    ...
    let safe_identifier =
      if not status.reserved_in_postgresql then identifier
        ...
        (* no identifier ending with "_" is an SQL keyword *)
        identifier ^ "_" in
    begin match warn with
      | `Silent -> ()
      | `Warning loc ->
        Syntax.print_warning loc
          (Printf.sprintf
             "The identifier %S is a reserved keyword %s, %s"
             identifier
             (match status.reserved_in_sql2003, status.reserved_in_postgresql
              with
                | true, true -> "in the 2003 SQL norm and in Postgresql"
                | false, true -> "in PostgreSQL (but not in the 2003 SQL norm)"
                | true, false -> "in the 2003 SQL norm (but not in Postgresql)"
                | false, false -> failwith "invalid reserved keyword status")
             (if status.reserved_in_postgresql then
                 Printf.sprintf "so %S will be used instead." safe_identifier
              else "it may be more future-proof to change it."))
    end;
    safe_identifier
...
let () =
  ...
  EXTEND CompGram
   ...
   (* one needs to "make safe" keywords at each occurence of them,
      but we don't wish a warning to be emitted each time for the
      same identifier. The sql_ident/sql_binder distinction allows
      to raise warnings for unsafe identifiers only when they are
      in binding position.
   *)
  sql_ident:
    [[ id = LIDENT -> keyword_safe `Silent id ]];
  sql_binder:
    [[ id = LIDENT -> keyword_safe (`Warning _loc) id ]];
  END;
